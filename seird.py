...
def step(people, infected, exposed, cell_size, infect_distance, infect_rate, recover_rate, progression_rate,
    ...
    return infected + new_infected, new_exposed, len(recover), len(deaths)
...
# SEIRD graph animation
def seird_model(y, t, beta, sigma, gamma, mu, radius, dimensions):
    S, E, I, R, D = y
    N = S + E + I + R
...
def create_SEIRD_simulation(width=500, height=500, number_of_people=5000, num_infected=50, infect_rate=0.8,
                            recover_rate=0.1, mortality_rate=0.03, progression_rate=0.2, infect_distance=10, max_move=5,
                            max_days=100, simulation_name="simulation1"):
    # Initialize people
    people = []
    infected = []
    exposed = []
    ...
    frames = []
    step_count = 1
    simulated_S, simulated_E, simulated_I, simulated_R, simulated_D = [number_of_people - num_infected], [0], [
        num_infected], [0], [0]

    while (len(infected) > 0 or len(exposed) > 0) and step_count <= max_days:
        step_count += 1
        screen.fill((0, 0, 0))
        infected, exposed, recovered_amount, deaths_amount = step(people, infected, exposed, infect_distance,
                                                                  infect_distance, infect_rate, recover_rate,
                                                                  progression_rate, mortality_rate)
        ...
        simulated_S.append(number_of_people - simulated_I[-1] - simulated_E[-1] - simulated_R[-1] - simulated_D[-1])
    ...
    initial_conditions = [simulated_S[0], simulated_E[0], simulated_I[0], simulated_R[0], simulated_D[0]]
    solution = odeint(seird_model, initial_conditions, t, args=(
        infect_rate, progression_rate, recover_rate, mortality_rate, infect_distance, (width, height)))
    S_model, E_model, I_model, R_model, D_model = solution.T
    ...
    ax.set_title("SEIRD Model Graph Animation")
    ax.set_xlabel("Days")
    ...
    def update(frame):
        for key, data in zip(lines.keys(),
                             [S_model, E_model, I_model, R_model, D_model, simulated_S, simulated_E, simulated_I,
