...
def step(people, infected, exposed, recovered, cell_size, infect_distance, infect_rate, recover_rate, progression_rate, lose_immunity_rate
         ):
    new_infected = []
    new_exposed = []
    new_recover = []
    ...
    for r in recovered:
        if random.random() < lose_immunity_rate:
            r[0] = 'S'
            lose_immune.append(r)
    # Update lists
    for r in new_recover:
        infected.remove(r)
    for s in lose_immune:
        recovered.remove(s)

    return infected + new_infected, new_exposed, len(new_recover), len(lose_immune)
...
# SEIRS graph animation
def seirs_model(y, t, beta, sigma, gamma, mu, radius, dimensions):
    S, E, I, R = y
    N = S + E + I + R
...
def create_SEIRS_simulation(width=500, height=500, number_of_people=5000, num_infected=50, infect_rate=0.8,
                            recover_rate=0.1, lose_immunity_rate=0.05, progression_rate=0.2, infect_distance=10, max_move=5,
                            simulation_name="simulation1", max_days=100):
    # Initialize people
    people = []
    infected = []
    exposed = []
    ...
    while (len(infected) > 0 or len(exposed) > 0) and step_count <= max_days:
        step_count += 1
        screen.fill((0, 0, 0))
        infected, exposed, recovered_amount, lose_immune_amount = step(people, infected, exposed, recovered,
                                                                       infect_distance, infect_distance, infect_rate,
                                                                       recover_rate, progression_rate, lose_immunity_rate)
        ...
        simulated_R.append(len(recovered))
    ...
    solution = odeint(seirs_model, initial_conditions, t, args=(
        infect_rate, progression_rate, recover_rate, lose_immunity_rate, infect_distance, (width, height)))
